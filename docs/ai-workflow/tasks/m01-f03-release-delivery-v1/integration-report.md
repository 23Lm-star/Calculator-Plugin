...
The local `integration` merge was completed as `dff723b`, then merged into the repository's main-equivalent `master` branch as `dd91e00`. `dd91e00` was pushed to `origin/1.1.0` and confirmed with `git ls-remote`. No tag or deployment was created.
...
The isolated worktree copied untracked build outputs only after matching Engine SHA-256 `A64C1FD5775E5019751E38062736076401EEF534526F5FE539C457AF9D7015B1` and application EXE SHA-256 `B1D0576D18B7B07109FF49368EC8BB32CB62DED5924A6AF93CB3597523D0C413`. Disk-capacity queries were denied by the managed sandbox, so capacity is unavailable rather than inferred. The verified `integration` merge `dff723b` was subsequently merged to `master` as `dd91e00` and pushed to `origin/1.1.0`; no tag or deployment occurred.
